...
const handler: Handler = async (event: HandlerEvent) => {
    ...
    try {
        ...
        if (!formData || !results || !user) {
            return { statusCode: 400, body: JSON.stringify({ error: 'Missing required payload data.' }) };
        }
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ error: "An internal error occurred while generating advice." }),
        };
    }
};
